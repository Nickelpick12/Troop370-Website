...
@Component({
  selector: 'app-scout-resourses-editor',
  templateUrl: './scout-resourses-editor.component.html',
  styleUrls: ['./scout-resourses-editor.component.css']
})
export class ScoutResoursesEditorComponent implements OnInit {
  ...
  loadPageData() {
    this.contentDbService.getPageData('scoutResourses').subscribe(doc => {
    ...
    this.contentDbService.getSensitivePageData('scoutResourses').subscribe(doc => {
      this.loaded.next(false);
      var nextPageData: PageData = {
        sections: doc.sections
      }
