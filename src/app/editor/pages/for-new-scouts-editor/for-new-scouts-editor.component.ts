...
@Component({
  selector: 'app-for-new-scouts-editor',
  templateUrl: './for-new-scouts-editor.component.html',
  styleUrls: ['./for-new-scouts-editor.component.css']
})
export class ForNewScoutsEditorComponent implements OnInit {
  ...
  loadPageData() {
    this.contentDbService.getPageData('forNewScouts').subscribe(doc => {
    ...
    this.contentDbService.getSensitivePageData('forNewScouts').subscribe(doc => {
      this.loaded.next(false);
      var nextPageData: PageData = {
        sections: doc.sections
      }
